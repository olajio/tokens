#!/usr/bin/env python3
"""
es_apikey_inventory_with_logstash.py
-------------------------------------
Discovers and inventories all Elasticsearch API keys across all clusters
defined in es_clusters.json, then cross-references each key against local
Logstash config files to identify which pipelines are using which keys.

Iterates dev → qa → ccs → prod (or whatever order clusters appear in config).
...
The "Used In (Script/Process)" column is populated automatically where a
Logstash config file references the token's ID or name.

Usage:
    python es_apikey_inventory_with_logstash.py
    python es_apikey_inventory_with_logstash.py --config /path/to/es_clusters.json
    python es_apikey_inventory_with_logstash.py --output my_inventory.xlsx
    python es_apikey_inventory_with_logstash.py --logstash-dirs /etc/logstash/conf.d /opt/logstash/pipeline
    python es_apikey_inventory_with_logstash.py --dump-json
    python es_apikey_inventory_with_logstash.py --list-clusters
...
# Default Logstash config directories scanned on the machine running this script.
# Override at runtime with --logstash-dirs.
DEFAULT_LOGSTASH_DIRS = [
    "/etc/logstash/conf.d",
    "/opt/logstash/pipeline",
]
...
def expiry_status(expiration_ms: Optional[int], invalidated: bool) -> str:
    if invalidated:
        return "INVALIDATED"
...
# Patterns that match Logstash api_key usage:
#   api_key => "id:value"   (Elasticsearch output plugin)
#   api_key: "id:value"     (YAML-style Filebeat/pipeline configs)
_LS_PATTERNS = [
...
def scan_logstash_dirs(dirs: List[str]) -> Dict[str, List[str]]:
    """
    Scan all .conf files under the given directories for api_key references.

    Returns:
        { token_id_or_name: ["/path/to/file1.conf", ...] }
    """
    ...
    for d in dirs:
        p = Path(d)
        if not p.exists():
            print(f"  ⚠️  Logstash dir not found, skipping: {d}")
            continue
        for conf_file in sorted(p.rglob("*.conf")):
            scanned += 1
            try:
                text = conf_file.read_text(errors="replace")
                for pattern in _LS_PATTERNS:
                    for m in pattern.finditer(text):
                        val = m.group(1).strip()
                        matches.setdefault(val, [])
                        if str(conf_file) not in matches[val]:
                            matches[val].append(str(conf_file))
            except Exception as e:
                print(f"  ⚠️  Could not read {conf_file}: {e}")

    print(f"  Scanned {scanned} Logstash config file(s), found {len(matches)} unique api_key reference(s)")
    return matches


def resolve_logstash_refs(key_id: str, key_name: str, ls_refs: Dict[str, List[str]]) -> str:
    """
    Match a token against the Logstash scan results.
    Checks both the token ID and token name since configs may use either.
    Returns a newline-joined list of matching file paths, or empty string.
    """
    files = set()
    # Token ID match (exact or as prefix of "id:value" compound key)
    for ref_key, ref_files in ls_refs.items():
        if ref_key == key_id or ref_key.startswith(key_id + ":"):
            files.update(ref_files)
    # Token name match
    if key_name in ls_refs:
        files.update(ls_refs[key_name])
    return "\n".join(sorted(files))
...
def fetch_api_keys(cluster_name: str, config: dict) -> list:
...
def normalize_row(cluster: str, key: dict, ls_refs: Dict[str, List[str]]) -> dict:
    ...
    return {
        ...
        "Purpose":                  "",
        "Team / Owner":             "",
        "Used In (Script/Process)": resolve_logstash_refs(key_id, key_name, ls_refs),
        "Source Location":          "",
        "Stored in PMP":            "No",
        "PMP Entry Name":           "",
        "Notes":                    "",
...
COL_WIDTHS = {
    ...
    "Used In (Script/Process)": 45, "Source Location": 30,
...
def _build_inventory_sheet(ws, rows: list, cluster_filter: Optional[str]):
    border      = _thin_border()
    ...
    ls_fill     = PatternFill("solid", fgColor="DEEAF1")   # light blue = auto-populated from Logstash
    ...
    for row_idx, row in enumerate(rows, start=2):
        has_multiline = "\n" in str(row.get("Permissions", "")) or \
                        "\n" in str(row.get("Used In (Script/Process)", ""))
        ...
        for col_idx, col_name in enumerate(COLUMNS, start=1):
            ...
            if col_name == "Status":
                ...
                if fill:
                    cell.fill = fill
            elif col_name == "Used In (Script/Process)":
                # Blue tint if auto-populated from Logstash scan; yellow if still empty
                cell.fill = ls_fill if value else manual_fill
            elif col_name in MANUAL_COLS and not value:
                cell.fill = manual_fill

        ws.row_dimensions[row_idx].height = 50 if has_multiline else 18
...
def _build_summary_sheet(ws, rows: list, clusters: list, logstash_dirs: list):
    ...
    ws["A3"] = f"Logstash scan: Yes  |  Dirs: {', '.join(logstash_dirs)}"
    ...
    ls_matched      = sum(1 for r in rows if r.get("Used In (Script/Process)"))
    ...
    flag_fills = {
        ...
        "Matched in Logstash configs": PatternFill("solid", fgColor="DEEAF1"),
    }
    ...
    for label, count in [
        ...
        ("Matched in Logstash configs",   ls_matched),
    ]:
...
def build_workbook(rows: list, output_path: str, cluster_names: list, logstash_dirs: list):
    wb = Workbook()

    ws_summary = wb.active
    ws_summary.title = "Summary"
    _build_summary_sheet(ws_summary, rows, cluster_names, logstash_dirs)
...
def main():
    parser = argparse.ArgumentParser(
        description="Elasticsearch API Key Inventory with Logstash config scanning"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--output", default="es_apikey_inventory_with_logstash.xlsx",
        help="Output Excel filename (default: es_apikey_inventory_with_logstash.xlsx)"
    )
    parser.add_argument(
        "--logstash-dirs", nargs="+", default=DEFAULT_LOGSTASH_DIRS,
        metavar="DIR",
        help=f"Logstash config directories to scan (default: {' '.join(DEFAULT_LOGSTASH_DIRS)})"
    )
    parser.add_argument(
    ...
    print("=" * 60)
    print("  Elasticsearch API Key Inventory + Logstash Scan")
    print(f"  Config:        {args.config}")
    print(f"  Logstash dirs: {', '.join(args.logstash_dirs)}")
    print("=" * 60)

    # Run Logstash scan once (configs are local to this machine, not per-cluster)
    print("\n🔎 Scanning Logstash config directories...")
    ls_refs = scan_logstash_dirs(args.logstash_dirs)
    ...
    for cluster_name, cluster_cfg in all_clusters.items():
        ...
        all_rows[cluster_name] = [normalize_row(cluster_name, k, ls_refs) for k in keys]

    flat_rows     = [row for rows in all_rows.values() for row in rows]
    cluster_names = [c.upper() for c in all_clusters.keys()]
    ls_hits       = sum(1 for r in flat_rows if r.get("Used In (Script/Process)"))

    print(f"\n📊 Total keys discovered: {len(flat_rows)}")
    print(f"   Matched to Logstash configs: {ls_hits}")
    ...
    build_workbook(flat_rows, args.output, cluster_names, args.logstash_dirs)
